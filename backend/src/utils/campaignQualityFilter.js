 ...
 * Bot'tan bağımsız, backend içinde self-contained
 */
...
function isHighQualityCampaign(campaign) {
  ...
  const descriptionLower = (campaign.description || '').toLowerCase().trim();
  ...
  // 4. Resmi link kontrolü
  const originalUrl = campaign.originalUrl || campaign.campaignUrl;
  if (!originalUrl || !_isOfficialUrl(originalUrl)) {
    return false;
  }
...
function _hasRealValue(campaign) {
  ...
  const description = (campaign.description || '').toLowerCase().trim();
