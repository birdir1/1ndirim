...
export default function SourcesPage() {
  ...
  return (
    <AdminLayout>
      ...
      <div className="mb-4 text-sm text-gray-600">{searchQuery ? `${list.length} sonuç bulundu` : `${list.length} kaynak gösteriliyor`}</div>
                    ...
                    <tr key={s.id} className="border-t hover:bg-gray-50">
                      <td className="p-2">
                        {s.logo_url ? (
                          ...
                          <img src={s.logo_url} alt={`${s.name} logo`} className="h-6 w-6 rounded object-contain bg-white" />
                        ) : (
                      ...
                      <td className="p-2 max-w-[220px] truncate">
                        {s.website_url ? (
                          <a href={s.website_url} target="_blank" rel="noreferrer" className="text-blue-600 hover:underline" title={s.website_url}>
                            {s.website_url}
                          </a>
                        ) : (
                          <span className="text-xs text-gray-400">-</span>
                      ...
                      <td className="p-2">
                        <Link href={`/scrapers/health?sourceId=${encodeURIComponent(s.id)}`} className="text-blue-600 hover:underline text-xs">
                          Health
                        </Link>
                      </td>
                      <td className="p-2">
                        {STATUS_OPTIONS.filter((x) => x !== (s.source_status || 'active')).map((st) => (
                          <button key={st} type="button" onClick={() => openModal(s, st)} className="mr-2 text-blue-600 hover:underline text-xs">
                            → {st}
                          </button>
                        ))}
                      </td>
              ...
              list.map((s) => (
                <div key={s.id} className="bg-white border rounded-lg p-4 shadow-sm">
                  <div className="flex items-center gap-3 mb-2">
                    {s.logo_url ? (
                      // eslint-disable-next-line @next/next/no-img-element
                      <img src={s.logo_url} alt={`${s.name} logo`} className="h-8 w-8 rounded object-contain bg-white" />
                    ) : (
                  ...
                  <div className="space-y-1 text-sm text-gray-600 mb-3">
                    ...
                    <div className="flex justify-between">
                      <span>Site:</span>
                      <span className="font-medium max-w-[220px] truncate text-right">
                        {s.website_url ? (
                          <a href={s.website_url} target="_blank" rel="noreferrer" className="text-blue-600 hover:underline">
                            {s.website_url}
                          </a>
                        ) : (
                          '-'
                    ...
                    <div className="flex justify-between">
                      <span>Aktif:</span>
                      <span className={`px-2 py-1 rounded text-xs font-medium ${s.is_active ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>
                        {s.is_active ? 'Evet' : 'Hayır'}
                      </span>
                    </div>
                    {s.status_reason && (
                  ...
                  <div className="flex flex-wrap gap-2 pt-2 border-t">
                    <Link
                      href={`/scrapers/health?sourceId=${encodeURIComponent(s.id)}`}
