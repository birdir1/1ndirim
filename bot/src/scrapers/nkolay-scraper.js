/**
 * N Kolay Campaign Scraper
 * N Kolay'ın public kampanya sayfasını okur
 */

const BaseScraper = require('./base-scraper');

class NKolayScraper extends BaseScraper {
  constructor() {
    super('N Kolay', 'https://www.nkolay.com/kampanyalar');
  }

  /**
   * N Kolay kampanyalarını scrape eder
   */
  async scrape() {
    const campaigns = [];

    try {
      // N Kolay kampanya sayfasını yükle
      ...
      const campaignLinks = await this.page.evaluate(() => {
        ...
        const links = allLinks
          .filter(a => {
            ...
            // N Kolay yapısı: /kampanyalar/ altında kampanyalar
          ...
          .map(a => {
            const href = a.href;
            const fullUrl = href.startsWith('http') ? href : `https://www.nkolay.com${href}`;
            return {
              href: fullUrl,
      ...
      throw new Error(`N Kolay scraper hatası: ${error.message}`);
    }
  }
  ...
  async parseCampaignFromLink(url, title) {
    try {
      ...
      return {
        ...
        title: content.title || title || 'N Kolay Kampanyası',
      ...
      return {
        sourceName: this.sourceName,
        title: title || 'N Kolay Kampanyası',
        description: title || '',
        detailText: '',
        campaignUrl: url,
        originalUrl: url,
