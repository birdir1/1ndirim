/**
 * Albaraka Türk Campaign Scraper
 * Albaraka Türk'ün public kampanya sayfasını okur
 */

const BaseScraper = require('./base-scraper');

class AlbarakaScraper extends BaseScraper {
  constructor() {
    super('Albaraka Türk', 'https://www.albaraka.com.tr/tr/kampanyalar');
  }

  /**
   * Albaraka Türk kampanyalarını scrape eder
   */
  async scrape() {
    const campaigns = [];

    try {
      // Albaraka Türk kampanya sayfasını yükle
      ...
      // Kampanya linklerini bul (Albaraka Türk için genişletilmiş selector)
      const campaignLinks = await this.page.evaluate(() => {
        ...
        const links = allLinks
          .filter(a => {
            ...
            // Albaraka Türk yapısı: /tr/kampanyalar/detay/ altında kampanyalar
            return (href.includes('/kampanyalar/detay/') || (href.includes('/kampanyalar/') && !href.endsWith('/kampanyalar')))
              && !href.includes('#')
          ...
          .map(a => {
            const href = a.href;
            const fullUrl = href.startsWith('http') ? href : `https://www.albaraka.com.tr${href}`;
            return {
              href: fullUrl,
      ...
      throw new Error(`Albaraka Türk scraper hatası: ${error.message}`);
    }
  }
  ...
  async parseCampaignFromLink(url, title) {
    try {
      ...
      return {
        ...
        title: content.title || title || 'Albaraka Türk Kampanyası',
      ...
      return {
        sourceName: this.sourceName,
        title: title || 'Albaraka Türk Kampanyası',
        description: title || '',
        detailText: '',
        campaignUrl: url,
        originalUrl: url,
...
module.exports = AlbarakaScraper;
