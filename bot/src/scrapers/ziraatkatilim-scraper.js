/**
 * Ziraat Katılım Campaign Scraper
 * Ziraat Katılım'ın public kampanya sayfasını okur
 */

const BaseScraper = require('./base-scraper');

class ZiraatKatilimScraper extends BaseScraper {
  constructor() {
    super('Ziraat Katılım', 'https://www.ziraatkatilim.com.tr/kampanyalar');
  }

  /**
   * Ziraat Katılım kampanyalarını scrape eder
   */
  async scrape() {
    const campaigns = [];

    try {
      // Ziraat Katılım kampanya sayfasını yükle
      ...
      const campaignLinks = await this.page.evaluate(() => {
        ...
        const links = allLinks
          .filter(a => {
            ...
            // Ziraat Katılım yapısı: /kampanyalar/ altında kampanyalar
          ...
          .map(a => {
            const href = a.href;
            const fullUrl = href.startsWith('http') ? href : `https://www.ziraatkatilim.com.tr${href}`;
            return {
              href: fullUrl,
      ...
      throw new Error(`Ziraat Katılım scraper hatası: ${error.message}`);
    }
  }
  ...
  async parseCampaignFromLink(url, title) {
    try {
      ...
      return {
        ...
        title: content.title || title || 'Ziraat Katılım Kampanyası',
      ...
      return {
        sourceName: this.sourceName,
        title: title || 'Ziraat Katılım Kampanyası',
        description: title || '',
        detailText: '',
        campaignUrl: url,
        originalUrl: url,
...
module.exports = ZiraatKatilimScraper;
