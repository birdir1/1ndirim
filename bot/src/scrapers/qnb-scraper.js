/**
 * QNB Finansbank Campaign Scraper
 * QNB Finansbank'ın public kampanya sayfasını okur
 */

const BaseScraper = require('./base-scraper');

class QNBScraper extends BaseScraper {
  constructor() {
    super('QNB Finansbank', 'https://www.qnbfinansbank.com/kampanyalar');
  }

  /**
   * QNB Finansbank kampanyalarını scrape eder
   */
  async scrape() {
    const campaigns = [];

    try {
      // QNB Finansbank kampanya sayfasını yükle
      ...
      throw new Error(`QNB Finansbank scraper hatası: ${error.message}`);
    }
  }
  ...
  async parseCampaignFromLink(url, title) {
    try {
      ...
      return {
        ...
        title: content.title || title || 'QNB Finansbank Kampanyası',
        ...
        tags: ['QNB Finansbank', subCategory].filter((t, i, a) => a.indexOf(t) === i),
        channel: 'online',
      };
    } catch (error) {
