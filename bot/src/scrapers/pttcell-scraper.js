/**
 * PTTcell Campaign Scraper
 * PTTcell'in public kampanya sayfasını okur
 */

const BaseScraper = require('./base-scraper');

class PTTcellScraper extends BaseScraper {
  constructor() {
    super('PTTcell', 'https://www.pttcell.com.tr/kampanyalar');
  }

  /**
   * PTTcell kampanyalarını scrape eder
   */
  async scrape() {
    const campaigns = [];

    try {
      // PTTcell kampanya sayfasını yükle
      ...
      const campaignLinks = await this.page.evaluate(() => {
        ...
        const links = allLinks
          .filter(a => {
            ...
            // PTTcell yapısı: /kampanyalar/ altında kampanyalar
            return (href.includes('/kampanyalar/') && !href.endsWith('/kampanyalar') && !href.endsWith('/kampanyalar/') && !href.endsWith('#'))
          ...
          .map(a => {
            const href = a.href;
            const fullUrl = href.startsWith('http') ? href : `https://www.pttcell.com.tr${href}`;
            return {
              href: fullUrl,
      ...
      throw new Error(`PTTcell scraper hatası: ${error.message}`);
    }
  }
  ...
  async parseCampaignFromLink(url, title) {
    try {
      ...
      return {
        ...
        title: content.title || title || 'PTTcell Kampanyası',
      ...
      return {
        sourceName: this.sourceName,
        title: title || 'PTTcell Kampanyası',
        description: title || '',
        detailText: '',
        campaignUrl: url,
        originalUrl: url,
