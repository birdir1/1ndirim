/**
 * CEPTETEB Campaign Scraper
 * CEPTETEB'in public kampanya sayfasını okur
 */

const BaseScraper = require('./base-scraper');

class CeptetebScraper extends BaseScraper {
  constructor() {
    super('CEPTETEB', 'https://www.cepteteb.com.tr/kampanyalar');
  }

  /**
   * CEPTETEB kampanyalarını scrape eder
   */
  async scrape() {
    const campaigns = [];

    try {
      // CEPTETEB kampanya sayfasını yükle
      ...
      const campaignLinks = await this.page.evaluate(() => {
        ...
        const links = allLinks
          .filter(a => {
            ...
            // CEPTETEB yapısı: /kampanyalar/cepteteb-kampanyalari/ altında kampanyalar
            return (href.includes('/kampanyalar/cepteteb-kampanyalari/') && !href.endsWith('/cepteteb-kampanyalari'))
              && !href.includes('#')
          ...
          .map(a => {
            const href = a.href;
            const fullUrl = href.startsWith('http') ? href : `https://www.cepteteb.com.tr${href}`;
            return {
              href: fullUrl,
      ...
      throw new Error(`CEPTETEB scraper hatası: ${error.message}`);
    }
  }
  ...
  async parseCampaignFromLink(url, title) {
    try {
      ...
      return {
        ...
        title: content.title || title || 'CEPTETEB Kampanyası',
      ...
      return {
        sourceName: this.sourceName,
        title: title || 'CEPTETEB Kampanyası',
        description: title || '',
        detailText: '',
        campaignUrl: url,
        originalUrl: url,
...
module.exports = CeptetebScraper;
