/**
 * Vakıf Katılım Campaign Scraper
 * Vakıf Katılım'ın public kampanya sayfasını okur
 */

const BaseScraper = require('./base-scraper');

class VakifKatilimScraper extends BaseScraper {
  constructor() {
    super('Vakıf Katılım', 'https://www.vakifkatilim.com.tr/tr/kendim-icin/kampanyalar/mevcut-kampanyalar');
  }

  /**
   * Vakıf Katılım kampanyalarını scrape eder
   */
  async scrape() {
    const campaigns = [];

    try {
      // Vakıf Katılım kampanya sayfasını yükle
      ...
      // Kampanya linklerini bul (Vakıf Katılım yapısı: /kampanyalar/detay/ altında kampanyalar)
      const campaignLinks = await this.page.evaluate(() => {
        ...
        const links = allLinks
          .filter(a => {
            ...
            // Vakıf Katılım yapısı: /kampanyalar/detay/ altında kampanyalar
            return (href.includes('/kampanyalar/detay/') && !href.includes('mevcut-kampanyalar'))
              && !href.includes('#')
          ...
          .map(a => {
            const href = a.href;
            const fullUrl = href.startsWith('http') ? href : `https://www.vakifkatilim.com.tr${href}`;
            return {
              href: fullUrl,
      ...
      throw new Error(`Vakıf Katılım scraper hatası: ${error.message}`);
    }
  }
  ...
  async parseCampaignFromLink(url, title) {
    try {
      ...
      return {
        ...
        title: content.title || title || 'Vakıf Katılım Kampanyası',
      ...
      return {
        sourceName: this.sourceName,
        title: title || 'Vakıf Katılım Kampanyası',
        description: title || '',
        detailText: '',
        campaignUrl: url,
        originalUrl: url,
...
module.exports = VakifKatilimScraper;
