/**
 * Emlak Katılım Campaign Scraper
 * Emlak Katılım'ın public kampanya sayfasını okur
 */

const BaseScraper = require('./base-scraper');

class EmlakKatilimScraper extends BaseScraper {
  constructor() {
    super('Emlak Katılım', 'https://www.emlakkatilim.com.tr/tr/bireysel/kampanyalar');
  }

  /**
   * Emlak Katılım kampanyalarını scrape eder
   */
  async scrape() {
    const campaigns = [];

    try {
      // Emlak Katılım kampanya sayfasını yükle
      ...
      // Kampanya linklerini bul (Emlak Katılım yapısı: /kampanyalar/kampanya/ altında kampanyalar)
      const campaignLinks = await this.page.evaluate(() => {
        ...
        const links = allLinks
          .filter(a => {
            ...
            // Emlak Katılım yapısı: /kampanyalar/kampanya/ altında kampanyalar
            return (href.includes('/kampanyalar/kampanya/') && !href.endsWith('/kampanyalar'))
              && !href.includes('#')
          ...
          .map(a => {
            const href = a.href;
            const fullUrl = href.startsWith('http') ? href : `https://www.emlakkatilim.com.tr${href}`;
            return {
              href: fullUrl,
      ...
      throw new Error(`Emlak Katılım scraper hatası: ${error.message}`);
    }
  }
  ...
  async parseCampaignFromLink(url, title) {
    try {
      ...
      return {
        ...
        title: content.title || title || 'Emlak Katılım Kampanyası',
      ...
      return {
        sourceName: this.sourceName,
        title: title || 'Emlak Katılım Kampanyası',
        description: title || '',
        detailText: '',
        campaignUrl: url,
        originalUrl: url,
...
module.exports = EmlakKatilimScraper;
