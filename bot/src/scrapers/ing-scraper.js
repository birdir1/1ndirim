/**
 * ING Bank Campaign Scraper
 * ING Bank'ın public kampanya sayfasını okur
 */

const BaseScraper = require('./base-scraper');

class IngScraper extends BaseScraper {
  constructor() {
    super('ING Bank', 'https://www.ing.com.tr/kampanyalar');
  }

  /**
   * ING Bank kampanyalarını scrape eder
   */
  async scrape() {
    const campaigns = [];

    try {
      // ING Bank kampanya sayfasını yükle
      ...
      throw new Error(`ING Bank scraper hatası: ${error.message}`);
    }
  }
  ...
  async parseCampaignFromLink(url, title) {
    try {
      ...
      return {
        ...
        title: content.title || title || 'ING Bank Kampanyası',
      ...
      return {
        sourceName: this.sourceName,
        title: title || 'ING Bank Kampanyası',
        description: title || '',
        detailText: '',
        campaignUrl: url,
        originalUrl: url,
