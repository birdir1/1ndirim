/**
 * Kuveyt Türk Campaign Scraper
 * Kuveyt Türk'ün public kampanya sayfasını okur
 */

const BaseScraper = require('./base-scraper');

class KuveytTurkScraper extends BaseScraper {
  constructor() {
    super('Kuveyt Türk', 'https://www.kuveytturk.com.tr/kampanyalar');
  }

  /**
   * Kuveyt Türk kampanyalarını scrape eder
   */
  async scrape() {
    const campaigns = [];

    try {
      // Kuveyt Türk kampanya sayfasını yükle
      ...
      throw new Error(`Kuveyt Türk scraper hatası: ${error.message}`);
    }
  }
  ...
  async parseCampaignFromLink(url, title) {
    try {
      ...
      return {
        ...
        title: content.title || title || 'Kuveyt Türk Kampanyası',
      ...
      return {
        sourceName: this.sourceName,
        title: title || 'Kuveyt Türk Kampanyası',
        description: title || '',
        detailText: '',
        campaignUrl: url,
        originalUrl: url,
...
module.exports = KuveytTurkScraper;
