/**
 * Tosla Campaign Scraper
 * PHASE 1: High-yield wallet scraper
 */

const BaseScraper = require('./base-scraper');

class ToslaScraper extends BaseScraper {
  constructor() {
    super('Tosla', 'https://www.tosla.com/kampanyalar');
  }
    ...
    try {
      ...
      const campaignLinks = await this.page.evaluate(() => {
        ...
        const links = allLinks
          ...
          .map(a => {
            const href = a.href;
            const fullUrl = href.startsWith('http') ? href : `https://www.tosla.com${href}`;
            return {
              href: fullUrl,
      ...
      throw new Error(`Tosla scraper hatası: ${error.message}`);
    }
  }
  ...
  async parseCampaignFromLink(url, title) {
    try {
      ...
      return {
        ...
        title: content.title || title || 'Tosla Kampanyası',
        ...
        category: 'finance',
        subCategory,
        tags: ['Tosla', subCategory].filter((t, i, a) => a.indexOf(t) === i),
        channel: 'online',
      };
    } catch (error) {
