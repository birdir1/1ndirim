...
class HalkbankScraper extends BaseScraper {
  constructor() {
    ...
    try {
      ...
      const campaignLinks = await this.page.evaluate(() => {
        ...
        const links = allLinks
          ...
          .map(a => {
            const href = a.href;
            const fullUrl = href.startsWith('http') ? href : `https://www.halkbank.com.tr${href}`;
            return {
              href: fullUrl,
              text: a.textContent.trim() || a.querySelector('h2, h3, h4, .title, .campaign-title, .card-title')?.textContent.trim() || '',
            };
          })
      ...
      throw new Error(`Halkbank scraper hatası: ${error.message}`);
    }
  }
...
module.exports = HalkbankScraper;
