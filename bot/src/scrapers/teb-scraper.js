/**
 * TEB Campaign Scraper
 * TEB'in public kampanya sayfasını okur
 */

const BaseScraper = require('./base-scraper');

class TebScraper extends BaseScraper {
  constructor() {
    super('TEB', 'https://www.teb.com.tr/sizin-icin/kampanyalar');
  }

  /**
   * TEB kampanyalarını scrape eder
   */
  async scrape() {
    const campaigns = [];

    try {
      // TEB kampanya sayfasını yükle
      ...
      // Kampanya linklerini bul (TEB için genişletilmiş selector)
      const campaignLinks = await this.page.evaluate(() => {
        ...
        const links = allLinks
          .filter(a => {
            ...
            // TEB yapısı: /sizin-icin/kampanyalar/ altında veya kampanya içeren linkler
            return (href.includes('/kampanyalar/') || href.includes('/kampanya/'))
              && !href.includes('#')
              && !href.includes('javascript:')
              && text.length > 5
              && !text.includes('kampanyalar') // Genel "kampanyalar" sayfasını hariç tut
              && !text.includes('kampanya arıyorum')
              && !text.includes('cepteteb');
          })
          .map(a => {
            const href = a.href;
            const fullUrl = href.startsWith('http') ? href : `https://www.teb.com.tr${href}`;
            return {
              href: fullUrl,
      ...
      throw new Error(`TEB scraper hatası: ${error.message}`);
    }
  }
  ...
  async parseCampaignFromLink(url, title) {
    try {
      ...
      return {
        ...
        title: content.title || title || 'TEB Kampanyası',
