    ...
    try {
      ...
      const campaignLinks = await this.page.evaluate(() => {
        ...
        return links
          ...
          .map(a => {
            const href = a.href;
            const fullUrl = href.startsWith('http') ? href : `https://bireysel.turktelekom.com.tr${href}`;
            return {
              href: fullUrl,
  ...
  async parseCampaignFromLink(url, title) {
    try {
      ...
      return {
        ...
        title: content.title || title || 'Türk Telekom Kampanyası',
      ...
      return {
        sourceName: this.sourceName,
        title: title || 'Türk Telekom Kampanyası',
        description: title || '',
        detailText: '',
        campaignUrl: url,
        originalUrl: url,
