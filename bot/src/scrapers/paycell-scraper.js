/**
 * Paycell Campaign Scraper
 * PHASE 1: High-yield wallet scraper
 */

const BaseScraper = require('./base-scraper');

class PaycellScraper extends BaseScraper {
  constructor() {
    super('Paycell', 'https://www.paycell.com.tr/kampanyalar');
  }
    ...
    try {
      ...
      const campaignLinks = await this.page.evaluate(() => {
        ...
        const links = allLinks
          ...
          .map(a => {
            const href = a.href;
            const fullUrl = href.startsWith('http') ? href : `https://www.paycell.com.tr${href}`;
            return {
              href: fullUrl,
      ...
      throw new Error(`Paycell scraper hatası: ${error.message}`);
    }
  }
  ...
  async parseCampaignFromLink(url, title) {
    try {
      ...
      return {
        ...
        title: content.title || title || 'Paycell Kampanyası',
        ...
        category: 'finance',
        subCategory,
        tags: ['Paycell', subCategory].filter((t, i, a) => a.indexOf(t) === i),
        channel: 'online',
      };
    } catch (error) {
