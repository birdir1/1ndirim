/**
 * VakıfBank Network Analyzer Script
 * FAZ 7.4: VakıfBank kampanya sayfası için network analizi
 */

const NetworkAnalyzer = require('../scrapers/fetch/network-analyzer');

async function analyzeVakifbank() {
  const analyzer = new NetworkAnalyzer(
    'VakıfBank',
    'https://www.vakifbank.com.tr/kampanyalar'
  );

  try {
    console.log('🔍 VakıfBank network analizi başlatılıyor...\n');
    ...
    if (results.allRequests.length > 0) {
      console.log(`\n📋 Tüm XHR/Fetch Request'ler (ilk 15):`);
      results.allRequests.slice(0, 15).forEach((req, index) => {
        console.log(`${index + 1}. ${req.method} ${req.url.substring(0, 100)}${req.url.length > 100 ? '...' : ''}`);
      });
    }
...
module.exports = analyzeVakifbank;
